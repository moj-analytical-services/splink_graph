...
def graphdensity(g,directed = False):
    ...
    def subgraph_stats(g,mysparksession=spark):
        ...
        subgraph_stats_schema = StructType([StructField("component", StringType()),StructField("density", FloatType()),StructField("graphsize", IntegerType())])
        cplist=[]
        denlist=[]
        sizelist=[]
        ...
        for i in list_of_subgraphs:
            ...
            current_component = i.select("component").distinct().rdd.flatMap(lambda x: x).collect()
        ...
        graphstats_df = mysparksession.createDataFrame(zip(cplist,denlist,sizelist),subgraph_stats_schema)
