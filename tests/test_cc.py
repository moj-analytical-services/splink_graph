...
def test_cc_simple(sparkwithgraphframes,graphframes_tmpdir):
    ...
    df_result =  sp_connected_components(e2_df,src="src",dst="dst",weight_colname="weight").toPandas()
    
    assert df_result["cluster_id"].count() == 2
    assert df_result["node_id"].unique().size == 2

    df_result2 =sp_connected_components(e2_df,src="src",dst="dst",weight_colname="weight",cc_threshold=0.2).toPandas()
